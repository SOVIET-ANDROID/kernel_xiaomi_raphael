...
setup_ak() {
  ...
  # slot detection enabled by is_slot_device=1 or auto (from kernelzip.sh)
  ...
  # target block partition detection enabled by block=boot recovery or auto (from kernelzip.sh)
  case $block in
     auto|"") block=boot;;
  esac;
  case $block in
